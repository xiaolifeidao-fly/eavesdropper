...
export class MonitorPxxSkuApi extends ElectronApi{
    ...
    async onGatherSkuMessage(callback: (gatherSku: GatherSku, updateSort: boolean) => void) {
    ...
    @InvokeType(Protocols.TRRIGER)
    async onGatherToolClosed(callback: () => void) {
      return await this.onMessage('onGatherToolClosed', callback)
    }
