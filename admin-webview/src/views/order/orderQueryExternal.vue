  ...
  <div class="tab-container" style="margin:1px;">
    <el-tabs v-model="shopCategoryId" tab-position="left" style="margin:0px;" type="border-card">
      <el-tab-pane v-for="item in tabShopOptions" :key="item.id" :label="item.name" :name="item.id">
        <keep-alive>
          <tab-pane-query-external v-if="shopCategoryId==item.id" :shop-category-id="item.id" @create="showCreatedTimes" />
        </keep-alive>
      </el-tab-pane>
    </el-tabs>
  </div>
...
import TabPaneQueryExternal from './components/TabPaneQueryExternal'

export default {
  name: 'Tab',
  components: { TabPaneQueryExternal },
  data() {
    return {
      tabShopOptions: [
        { label: 'China2', key: 'CN' }
